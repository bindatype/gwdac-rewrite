...
NR == FNR {
    ...
    typed_input[row] = $16
    typed_effective[row] = $17
    typed_dither[row] = $18
    typed_hadronic_real[row] = $29
    typed_hadronic_imag[row] = $30
    next
}
...
FNR == 1 {
    print "row_id", "family", "branch", "legacy_l", "state_index", \
        ...
        "legacy_previous", "typed_pre_reset", "legacy_epx", \
        "typed_input", "legacy_epxx", "typed_effective", \
        "legacy_effective", "typed_dither", "legacy_dither", \
        "typed_hadronic_real", "legacy_hadronic_real", \
        "typed_hadronic_imag", "legacy_hadronic_imag", "exact", \
        "first_field"
    next
}
...
{
    row = $2
    legacy_title = normalized_title($14)
    legacy_previous = normalized_title($15)
    first_field = ""
    ...
    else if ($12 == "T" && legacy_previous != typed_previous[row]) \
        first_field = "previous_title"
    else if ($12 == "T" && $11 != typed_input[row]) \
        first_field = "input_energy"
    else if ($12 == "T" && $16 != typed_effective[row]) \
        first_field = "effective_energy"
    else if ($12 == "T" && $17 != typed_dither[row]) \
        first_field = "dither"
    else if ($12 == "T" && $18 != typed_hadronic_real[row]) \
        first_field = "hadronic_real"
    else if ($12 == "T" && $19 != typed_hadronic_imag[row]) \
        first_field = "hadronic_imag"

    print row, $3, $4, $5, $8, $9, typed_dispatch[row], $12, \
        typed_title[row], legacy_title, typed_previous[row], \
        legacy_previous, typed_pre_reset[row], $10, typed_input[row], $11, \
        typed_effective[row], $16, typed_dither[row], $17, \
        typed_hadronic_real[row], $18, typed_hadronic_imag[row], $19, \
        (first_field == "" ? "yes" : "no"), \
