...
index($0, "CM12DIAG PRDLT") && !index($0, "CM12DIAG PRDLT FIELDS") {
    ...
    pre_reset_energy[key] = field[10]
    post_reset_energy[key] = field[11]
    epxx[key] = field[12]
}
...
END {
    print "sequence", "row_id", "family", "branch", "legacy_l", \
        ...
        "pre_reset_energy", "post_reset_energy", "epxx", "dispatch", \
        "dispatch_branch", "dispatch_title", "previous_title", \
        "effective_energy", "dither_before", "hadronic_real", \
        "hadronic_imag"
    for (current_sequence = 1; current_sequence <= sequence; \
        ...
        for (row = 1; row <= maximum_row[current_sequence]; row++) {
            ...
            print current_sequence, row, family[key], branch[key], \
                ...
                state_index[key], form_selector[key], \
                pre_reset_energy[key], post_reset_energy[key], epxx[key], \
